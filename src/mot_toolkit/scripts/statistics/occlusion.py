...
import itertools  # 添加这一行，用于循环颜色和标记
...
class OcclusionAnalyzer:
    ...
    def analyze_occlusions(self):
        ...
        for (
            object_id,
            frames,
        ) in self.object_frames.items():  # tqdm removed for less verbose parallel
            frame_indices = sorted(
                [
                    idx
                    for idx in [frame_id_to_index.get(f, -1) for f in frames]
                    if idx != -1
                ]
            )
            ...
            self.occlusion_stats[object_id] = {
                ...
                "gap_count": gap_count,
                "gaps": gaps,
                "gap_durations": current_gap_durations,
            }
    ...
    def generate_summary(self) -> Dict:
        ...
        all_durations = [
            ...
            for dur in stats["gap_durations"]
        ]
        return {
            "sequence_name": self.sequence_name,
    ...
    def plot_gap_count_line_chart(self):
        ...
        # 使用不同标记和颜色
        colors = self.color_scheme.hex_colors()
        markers = ["o", "s", "^", "x", "D", "*", "+", "v", "<", ">", "p", "h"]

        # 先绘制连接线
        plt.plot(x, y, linewidth=2, color=colors[0], alpha=0.7)
        ...
        for i, (px, py) in enumerate(zip(x, y)):
            ...
            if py > 0:
                plt.text(px, py + max(y) * 0.03, str(py), ha="center", fontsize=10)
        ...
        output_path = os.path.join(
            self.output_dir, f"{self.sequence_name}_gap_count_line.svg"
        )
        plt.tight_layout()
    ...
    def plot_gap_duration_histogram(self, bin_edges=None):
        ...
        if bin_edges is None:
            ...
            # 修改bin_edges创建逻辑，使0-20范围更细化
            if max_duration <= 20:
                bin_edges = list(range(max_duration + 2))  # 每一帧都是一个区间
            else:
                # 0-20的部分细化，每一帧一个区间
                bin_edges = list(range(21))
                # 20以上的部分根据最大值进行稀疏化
                if max_duration <= 50:
                    bin_edges.extend(range(25, max_duration + 6, 5))
                elif max_duration <= 100:
                    bin_edges.extend([25, 30, 40, 50, 60, 70, 80, 90, 100])
                    if max_duration > 100:
                        bin_edges.append(max_duration + 1)
                else:
                    bin_edges.extend([25, 30, 40, 50, 75, 100, 150, 200, 300, 500])
                    if max_duration > 500:
                        bin_edges.append(max_duration + 1)
                # 确保最后一个bin可以覆盖到最大值
        ...
        # 为每个柱子分配颜色，按顺序交替使用
        bar_colors = [colors_list[i % len(colors_list)] for i in range(len(hist))]
        ...
        plt.ylim(0, y_max * 1.15)
        output_path = os.path.join(
            self.output_dir, f"{self.sequence_name}_gap_duration_hist.svg"
        )
        plt.tight_layout()
        plt.savefig(output_path, format="svg", bbox_inches="tight")
        plt.close()
    ...
    def plot_occlusion_ratio_pie(self):
        ...
        output_path = os.path.join(
            self.output_dir, f"{self.sequence_name}_occlusion_ratio_pie.svg"
        )
        plt.tight_layout()
    ...
    def create_dashboard(self):
        ...
        if durations:
            max_d = max(durations)
            if max_d <= 20:
                bin_e = list(range(max_d + 2))
            else:
                bin_e = list(range(21))
                if max_d <= 50:
                    bin_e.extend(range(25, max_d + 6, 5))
                elif max_d <= 100:
                    bin_e.extend([25, 30, 40, 50, 60, 75, 100])
                else:
                    bin_e.extend([25, 30, 40, 50, 75, 100, 150, 200, 300, 500, 1000])
                if bin_e[-1] < max_d:
                    bin_e.append(int(max_d) + 1)
                bin_e = sorted(list(set(bin_e)))
            hist, _ = np.histogram(durations, bins=bin_e)
            bin_l = [
                (
                    f"{bin_e[i]}-{bin_e[i+1]-1}"
                    if bin_e[i + 1] - bin_e[i] > 1
                    else f"{bin_e[i]}"
                )
            ...
            colors_list = self.color_scheme.hex_colors()
            bar_colors = (
                colors_list[1 : len(hist) + 1]
                if len(hist) <= (len(colors_list) - 1)
                else colors_list[1]
            )
            ...
            ax2.set_xticklabels(bin_l, rotation=45)
        ...
        for i, (oid, s) in enumerate(top_occ):
            if s["occlusion_ratio"] > 0:
                info += f"{i+1}. ID {oid}: Occ {s['occlusion_ratio']:.1%}, Gaps {s['gap_count']}\n"
        ax4.text(0.1, 0.9, info, va="top", fontsize=10, transform=ax4.transAxes)

        plt.tight_layout(rect=[0, 0.03, 1, 0.95])
        output_path = os.path.join(
            self.output_dir, f"{self.sequence_name}_occlusion_dashboard.svg"
        )
        plt.savefig(output_path, format="svg", bbox_inches="tight")
        plt.close()
    ...
    def run_analysis(self) -> Tuple[Optional[Dict], Optional[Dict]]:
        ...
        if not self.load_data():
            return None, None
        if not self.analyze_occlusions():
            return None, None
        ...
        has_gap_data = any(
            count > 0
            for count in summary["gap_count_distribution"].values()
            if count != summary["gap_count_distribution"].get(0, 0)
        ) or (
            summary["gap_count_distribution"].get(0, 0) != len(self.occlusion_stats)
...
def plot_global_occlusion_duration_histogram(
    all_durations: List[int], output_dir: str, bins: int = 20
):
    ...
    print(f"全局：绘制遮挡持续时长柱状图，自定义区间分布...")

    max_duration = max(all_durations) if all_durations else 0

    # 修改全局柱状图的区间划分逻辑，更细化0-20帧区间
    if max_duration == 0 and not all_durations:
        bin_edges_calc = [0, 1]
        ...
        # 0-20部分每一帧一个区间
        bin_edges_calc = list(range(21))

        # 根据最大持续时长决定后续区间划分
        if max_duration <= 50:
            # 20-50部分，每5帧一个区间
            bin_edges_calc.extend(range(25, max_duration + 6, 5))
        elif max_duration <= 100:
            # 20-100部分，区间逐渐变大
            bin_edges_calc.extend([25, 30, 40, 50, 60, 75, 100])
        else:
            # 大于100，区间更稀疏
            bin_edges_calc.extend([25, 30, 40, 50, 75, 100, 150, 200, 300, 500, 1000])

        # 确保最后一个bin_edge覆盖最大值
        if bin_edges_calc[-1] < max_duration:
            bin_edges_calc.append(int(max_duration) + 1)

        # 确保bin_edges_calc是递增且唯一的
    ...
    for i in range(len(actual_bin_edges) - 1):
        ...
        if end - start == 1:
            bin_labels.append(f"{int(start)}")
        else:
            bin_labels.append(f"{int(start)}-{int(end-1)}")

    plt.figure(figsize=(12, 8))
    # 使用希格雯配色，每个柱子按顺序使用不同颜色
    colors = SIGEWINNEColorScheme().hex_colors()
    bar_colors = [colors[i % len(colors)] for i in range(len(hist))]
    ...
    for bar, count in zip(bars, hist):
        if count > 0:
            plt.text(
                bar.get_x() + bar.get_width() / 2,
                bar.get_height() + max(hist) * 0.02 if max(hist) > 0 else 0.1,
                str(int(count)),
                ha="center",
                fontsize=10,
            )
    ...
    plt.ylim(0, y_max * 1.15)
    plt.tight_layout()
    output_path = os.path.join(output_dir, "global_occlusion_duration_histogram.svg")
    plt.savefig(output_path, format="svg", bbox_inches="tight")
    plt.close()
    print(f"全局：已保存遮挡持续时长柱状图到: {output_path}")


def plot_global_occlusion_count_line_chart(
    aggregated_counts: Dict[int, int], output_dir: str
):
    """绘制全局遮挡次数折线图"""
    ...
    x_coords = sorted(
        [k for k, v in aggregated_counts.items() if v > 0]
    )  # Only plot if objects exist for that count
    if not x_coords:
    ...
    # 先绘制连接线
    plt.plot(x_coords, y_coords, linewidth=2, color=colors[0], alpha=0.7)
    ...
    for i, (px, py) in enumerate(zip(x_coords, y_coords)):
        ...
        if py > 0:
            plt.text(
                px,
                py + max(y_coords) * 0.03 if max(y_coords) > 0 else 0.1,
                str(py),
                ha="center",
                fontsize=10,
            )
    ...
    plt.grid(True, linestyle="--", alpha=0.7)
    plt.xticks(x_coords, fontsize=12)
    y_max = max(y_coords) if y_coords else 1
    plt.ylim(0, y_max * 1.15)
    plt.tight_layout()
    output_path = os.path.join(output_dir, "global_occlusion_count_line_chart.svg")
    plt.savefig(output_path, format="svg", bbox_inches="tight")
    plt.close()
...
def plot_global_occlusion_ratio_pie(all_object_ratios: List[float], output_dir: str):
    ...
    # 使用希格雯配色，确保每个扇区使用一个颜色
    colors = SIGEWINNEColorScheme().hex_colors()
    pie_colors = [colors[i % len(colors)] for i in range(len(sizes))]
    ...
    wedges, texts, autotexts = plt.pie(
        ...
        labeldistance=1.1,
        pctdistance=0.75,  # 调整标签和百分比的位置以避免重叠
    )
    # 调整标签文字
    for text in texts:
    ...
    plt.axis("equal")
    plt.title("Global Object Occlusion Ratio Distribution", fontsize=16, y=1.05)
    plt.tight_layout()
    output_path = os.path.join(output_dir, "global_occlusion_ratio_pie.svg")
    plt.savefig(output_path, format="svg", bbox_inches="tight")
    plt.close()
...
def generate_combined_report(
    ...
    main_output_dir: str,
    hist_bins: int = 20,  # 添加参数，控制柱状图区间数
):
    """
    ...
    Args:
        ...
        hist_bins: 柱状图的区间数量 - 注意: 现在使用自定义区间而非固定数量
    """
    ...
    for key, summary, seq_occlusion_stats in processed_data:
        ...
        if seq_occlusion_stats:  # Should always be true if it's in processed_data
            for obj_id, stats_data in seq_occlusion_stats.items():
                all_individual_object_ratios.append(
                    stats_data.get("occlusion_ratio", 0.0)
                )
    ...
    summary_csv_path = os.path.join(
        main_output_dir, "combined_occlusion_summary_sequences.csv"
    )
    with open(summary_csv_path, "w", newline="", encoding="utf-8") as csvfile:
        ...
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()
        for key, summary, _ in sorted(
            processed_data, key=lambda x: x[1].get("occlusion_ratio", 0), reverse=True
        ):
            writer.writerow(
                {
                    "Sequence": key,
    ...
    plot_global_occlusion_duration_histogram(
        all_gap_durations_list, main_output_dir, bins=hist_bins
    )
    ...
    summary_txt_path = os.path.join(
        main_output_dir, "combined_occlusion_summary_report.txt"
    )
    with open(summary_txt_path, "w", encoding="utf-8") as txtfile:
        txtfile.write("=== Combined Occlusion Analysis Summary Report ===\n\n")
        txtfile.write(
            f"Number of Sequences Processed (with occlusions): {len(processed_data)}\n"
        )
        txtfile.write(
            f"Total Objects Across These Sequences: {total_summary_objects}\n"
        )
        txtfile.write(
            f"Total Objects with Any Occlusion: {total_summary_objects_with_occlusion}\n"
        )

        overall_occlusion_obj_ratio = 0
        if total_summary_objects > 0:
            overall_occlusion_obj_ratio = (
                total_summary_objects_with_occlusion / total_summary_objects * 100
            )
        txtfile.write(
            f"Overall Ratio of Objects Experiencing Occlusion: {overall_occlusion_obj_ratio:.2f}%\n"
        )
        txtfile.write(
            f"Total Occluded Frames Across All Objects: {total_summary_occluded_frames}\n\n"
        )

        if all_gap_durations_list:
            avg_duration = sum(all_gap_durations_list) / len(all_gap_durations_list)
            max_d = max(all_gap_durations_list)
            min_d = min(all_gap_durations_list)
...
def parse_args():
    ...
    parser.add_argument(
        "--hist-bins", type=int, default=20, help="柱状图的区间数量 (默认: 20)"
    )
...
def main():
    ...
    if args.output:
        ...
        main_output_dir = os.path.join(
            script_dir, "output", "occlusion_analysis", dataset_name
        )
    ...
    if os.path.exists(
        main_output_dir
    ):  # Clean main_output_dir for a fresh run of combined report
        print(
            f"警告：主输出目录 {main_output_dir} 已存在，将清空其中的临时文件并重新生成报告。"
        )
    ...
    if args.black_list:
        ...
        video_dir_list = [
            vd
            for vd in video_dir_list
            if not any(keyword in vd for keyword in args.black_list)
        ]
        print(
            f"应用黑名单，从 {original_count} 个视频目录过滤后剩余 {len(video_dir_list)} 个。"
        )
    ...
    if not video_dir_list:
        ...
        is_video_dir = all(
            os.path.isdir(os.path.join(input_base_dir, item))
            for item in os.listdir(input_base_dir)
        ) and not any(
            item.endswith(".json") for item in os.listdir(input_base_dir)
        )  # Heuristic
        ...
        if is_sequence_dir:  # input_base_dir is a single sequence directory
            ...
            dummy_video_name = (
                parent_dir_name if parent_dir_name != dataset_name else "single_video"
            )

            seq_temp_dir = os.path.join(
                temp_overall_dir, dummy_video_name, seq_name_for_key
            )
            ...
            print(
                f"开始处理 {len(all_sequence_dirs_tasks)} 个序列，使用 {cpu_to_use} 个进程..."
            )
            with multiprocessing.Pool(processes=cpu_to_use) as pool:
                results_from_pool = list(
                    tqdm(
                        pool.imap(process_single_sequence, all_sequence_dirs_tasks),
                        total=len(all_sequence_dirs_tasks),
                        desc="处理序列",
                    )
                )
            ...
            print(
                f"在 {input_base_dir} 中没有找到有效的视频目录或序列。请检查输入路径。"
            )
        ...
        for video_dir_path in video_dir_list:
            ...
            for seq_subdir_name in sequence_subdirs:
                ...
                sequence_temp_output_dir = os.path.join(
                    temp_overall_dir, video_name, seq_subdir_name
                )
        ...
        cpu_to_use = (
            args.processes
            if args.processes
            else max(1, multiprocessing.cpu_count() // 2 - 1)
        )
        print(
            f"总计发现 {total_sequences_to_process} 个序列，开始使用 {cpu_to_use} 个进程进行并行处理..."
        )
    ...
    if processed_results:
        generate_combined_report(
            processed_results, main_output_dir, args.hist_bins
        )  # 传递区间数参数
        print("注意：所有图像已保存为SVG格式，提供更好的矢量图质量")
    else:
