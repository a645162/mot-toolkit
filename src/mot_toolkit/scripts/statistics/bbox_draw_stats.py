...
def draw_sequence_bboxes_tensor(
    ...
    # 生成随机颜色
    color = torch.tensor([0.0, 0.6, 1.0], device=device)  # 使用蓝色
    ...
    for norm_width, norm_height in normalized_sizes:
        ...
        if right > left and bottom > top:
            ...
            if top + line_width <= bottom:
                canvas[top : top + line_width, left:right, :] += color * alpha

            # 绘制下边框
            if bottom - line_width >= top:
                canvas[bottom - line_width : bottom, left:right, :] += color * alpha

            # 绘制左边框
            if left + line_width <= right:
                canvas[top:bottom, left : left + line_width, :] += color * alpha

            # 绘制右边框
            if right - line_width >= left:
                canvas[top:bottom, right - line_width : right, :] += color * alpha
...
def combine_canvases(
    ...
    if not canvas_results:
        return torch.zeros((512, 512, 3), dtype=torch.float32)
...
def parse_args():
    ...
    parser.add_argument("--alpha", type=float, default=0.005, help="每个bbox的透明度")
    parser.add_argument("--max-workers", type=int, default=8, help="最大工作线程数")
    parser.add_argument(
        "--output", type=str, default="bbox_distribution.jpg", help="输出图像文件名"
    )
    parser.add_argument(
        "--device",
        type=str,
