...
def handle_sequence(
    ...
    stop_early = stop_early and len(label_list) > 0

    # 根据详细程度决定日志输出级别
    if verbose:
        logger.info(f"Processing sequence: {sequence_dir_path}")
        logger.info(
            f"Parameters: iou_threshold={iou_threshold}, model_name={model_name}"
        )
        logger.info(
            f"Frame range: start={start_frame}, end={end_frame}, iterations={times}"
        )
        if label_list:
            logger.info(f"Processing labels: {', '.join(label_list)}")
    ...
    if verbose:
        logger.info(
            f"Found {len(annotation_directory.file_path_list)} files in sequence directory"
        )
    ...
    if verbose:
        logger.info(
            f"Loaded {len(annotation_directory.annotation_file_list)} annotation files"
        )

    device = wait_gpu_memory(memory_size="3GiB", time_interval=2)

    if verbose:
        logger.info(f"Using device: {device}")

    model = sam2.sam_load(
        target_device=device, global_mode=False, model_name=model_name
    )

    if verbose:
        logger.info(f"SAM model loaded: {model_name if model_name else 'default'}")

    # 执行多次SAM处理
    for iteration in range(times):
        logger.info(f"Running SAM iteration {iteration + 1}/{times}")
        ...
        for annotation_file_obj in tqdm.tqdm(annotation_directory.annotation_file_list):
            ...
            if -1 < end_frame < current_file_index:
                if verbose:
                    logger.info(f"Reached end frame {end_frame}, breaking")
                break

            if len(first_file_name) > 0:
                if first_file_name.endswith(".json") or first_file_name.endswith(
                    ".jpg"
                ):
            ...
            if verbose:
                logger.info(f"Processing annotation file: {current_file_name}")
            ...
            for rect_obj in annotation_file_obj.rect_annotation_list:
                if len(label_list) > 0:
                    if rect_obj.label not in label_list:
                        if stop_early:
                            if verbose:
                                logger.info(
                                    f"Label {rect_obj.label} not in target list, early stopping"
                                )
                            break
                        continue
                ...
                if copy_previous and previous_file_obj is not None:
                    ...
                    for previous_rect_obj in previous_file_obj.rect_annotation_list:
                        if rect_obj.label == previous_rect_obj.label:
                            ...
                            if verbose:
                                logger.info(
                                    f"Copied object {rect_obj.label} from frame "
                                    f"{previous_file_obj.file_name_no_extension} to "
                ...
                if verbose:
                    logger.info(f"Processing object {rect_obj.label} at {bbox_xyxy}")
                ...
                if len(result) > 0:
                    ...
                    if verbose:
                        logger.info(f"Updated object {rect_obj.label} to {result}")
                else:
                    message = f"No result for Object:{rect_obj.label} {image_path}"
            ...
            if verbose and objects_processed > 0:
                logger.info(
                    f"Processed {objects_processed} objects in frame {current_file_name}"
                )
        ...
        if verbose:
            logger.info(
                f"Iteration {iteration + 1}/{times} completed: processed {processed_count} files, modified {modified_count} files"
            )
    ...
    if verbose:
        logger.info(
            f"All {times} iterations completed and results saved to {sequence_dir_path}"
        )
