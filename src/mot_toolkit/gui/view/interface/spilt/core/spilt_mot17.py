...
class SpiltMOT17:
    ...
    def handle_mot17_sequence_param(
            self,
            sequence_dir: str,
            target_dir: str
    ):
    ...
    def output_mot17(
            self,
            output_mot17_dir: str,
            dataset_train_list: List[DatasetSpilt],
            dataset_val_list: List[DatasetSpilt],
            dataset_test_list: List[DatasetSpilt],
    ):
        ## Train
        mot17_train_dir = os.path.join(output_mot17_dir, "train")
        os.makedirs(mot17_train_dir, exist_ok=True)
        self.output_mot17_spilt(
            dataset_list=dataset_train_list,
            output_dir=mot17_train_dir
        )

        ## Val
        mot17_val_dir = os.path.join(output_mot17_dir, "val")
        os.makedirs(mot17_val_dir, exist_ok=True)
        self.output_mot17_spilt(
            dataset_list=dataset_val_list,
            output_dir=mot17_val_dir
        )

        ## Test
        mot17_test_dir = os.path.join(output_mot17_dir, "test")
        os.makedirs(mot17_test_dir, exist_ok=True)
        self.output_mot17_spilt(
            dataset_list=dataset_test_list,
            output_dir=mot17_test_dir
        )

        task_count = len(self.multiprocess_task_params)

        logger.info("Output MOT17 Task Generate Done!")
        logger.info("Output MOT17 Task Count: " + str(task_count))
        ...
        if self.multiprocess_mode and self.process_count > 1:
            ...
            for param in self.multiprocess_task_params:
                self.handle_mot17_sequence(*param)
        logger.info("Output MOT17 format finished!")

        logger.info("Generate SeqMap")
        train_seq_map_path = os.path.join(output_mot17_dir, "train_seqmap.txt")
        self.generate_seq_map(
            dir_path=os.path.join(output_mot17_dir, "train"),
            save_path=train_seq_map_path
        )
        logger.info("Generate Train Done! " + train_seq_map_path)

        val_seq_map_path = os.path.join(output_mot17_dir, "val_seqmap.txt")
        self.generate_seq_map(
            dir_path=os.path.join(output_mot17_dir, "val"),
            save_path=val_seq_map_path
        )
        logger.info("Generate Val Done! " + val_seq_map_path)

        test_seq_map_path = os.path.join(output_mot17_dir, "test_seqmap.txt")
        self.generate_seq_map(
            dir_path=os.path.join(output_mot17_dir, "test"),
        ...
        logger.info("Output MOT17 Task Done!")
