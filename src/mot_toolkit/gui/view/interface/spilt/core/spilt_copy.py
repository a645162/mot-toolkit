...
class SpiltLabelme:
    ...
    def handle_labelme_sequence(self, sequence_dir: str, target_dir: str):
        if not os.path.exists(sequence_dir):
            logger.error("Source Sequence Dir Not Found: " + sequence_dir)
            return
        ...
        try:
            ...
            for item in os.listdir(sequence_dir):
                source_item = os.path.join(sequence_dir, item)
            ...
            logger.info(f"Successfully copied {sequence_dir} to {target_dir}")
        except Exception as e:
            logger.error(f"Error copying directory: {str(e)}")

    def handle_labelme_sequence_param(self, sequence_dir: str, target_dir: str):
        return (sequence_dir, target_dir)

    def handle_labelme_dir(self, dataset_dir_obj: DatasetSpilt, output_dir: str):
        source_dir = dataset_dir_obj.abs_path
        ...
        # depth=1 确保只获取直接子目录，即序列目录
        sequence_path_list = get_dataset_dir_list(dataset_dir_path=source_dir, depth=1)
        for sequence_path in sequence_path_list:
            # 生成重命名后的目录名，确保在新的1级结构中不会冲突
            new_sequence_name = dataset_dir_obj.generate_new_name(sequence_path)
            target_dir = os.path.join(output_dir, new_sequence_name)

            logger.info(
                f"将序列 {os.path.basename(sequence_path)} 重命名为 {new_sequence_name}"
            )

            self.multiprocess_task_params.append(
                self.handle_labelme_sequence_param(
                    sequence_dir=sequence_path, target_dir=target_dir
                )
            )

    def output_labelme_spilt(self, dataset_list: List[DatasetSpilt], output_dir: str):
        logger.info("Output Labelme")
        logger.info("Dataset List Count: " + str(len(dataset_list)))
        logger.info("Output Dir: " + output_dir)

        for dataset_obj in dataset_list:
            self.handle_labelme_dir(dataset_dir_obj=dataset_obj, output_dir=output_dir)

    def output_labelme(
        self,
        output_labelme_dir: str,
        dataset_train_list: List[DatasetSpilt],
        dataset_val_list: List[DatasetSpilt],
        dataset_test_list: List[DatasetSpilt],
    ):
        ...
        # Train
        labelme_train_dir = os.path.join(output_labelme_dir, "train")
        os.makedirs(labelme_train_dir, exist_ok=True)
        self.output_labelme_spilt(
            dataset_list=dataset_train_list, output_dir=labelme_train_dir
        )

        # Val
        labelme_val_dir = os.path.join(output_labelme_dir, "val")
        os.makedirs(labelme_val_dir, exist_ok=True)
        self.output_labelme_spilt(
            dataset_list=dataset_val_list, output_dir=labelme_val_dir
        )

        # Test
        labelme_test_dir = os.path.join(output_labelme_dir, "test")
        os.makedirs(labelme_test_dir, exist_ok=True)
        self.output_labelme_spilt(
            dataset_list=dataset_test_list, output_dir=labelme_test_dir
        )

        task_count = len(self.multiprocess_task_params)

        logger.info("Output Labelme Task Generate Done!")
        logger.info("Output Labelme Task Count: " + str(task_count))

        logger.info("Start task on " + str(self.process_count) + " CPU Cores")
        if self.multiprocess_mode and self.process_count > 1:
            with multiprocessing.Pool(processes=self.process_count) as pool:
                pool.starmap(
                    self.handle_labelme_sequence, self.multiprocess_task_params
                )
        else:
            for param in self.multiprocess_task_params:
                self.handle_labelme_sequence(*param)

        logger.info("Output Labelme Task Done!")
