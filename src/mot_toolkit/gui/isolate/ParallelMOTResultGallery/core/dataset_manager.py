        ...
        for split in self.selected_splits:
            ...
            for item in items:
                ...
                if os.path.isdir(seq_path):
                    ...
                    has_img1 = 'img1' in all_files and os.path.isdir(os.path.join(seq_path, 'img1'))
                    has_gt = 'gt' in all_files and os.path.isdir(os.path.join(seq_path, 'gt'))
                    has_seqinfo = 'seqinfo.ini' in all_files
                    
                    print(f"[DatasetManager] 在{seq_path}中找到文件: {all_files}")
                    print(f"[DatasetManager] 检查DanceTrack格式: img1={has_img1}, gt={has_gt}, seqinfo.ini={has_seqinfo}")
                    
                    # 只要是有效的DanceTrack序列就添加
                    if has_img1:
                        # 使用序列名作为key，完整路径作为value
                        self.sequence_paths[item] = seq_path
                        print(f"[DatasetManager] 添加DanceTrack序列: {item} -> {seq_path}")
